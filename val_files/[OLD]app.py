...
unique_cities = weather_data["city"].unique()
    ...
    if selected_tab == "tab1":
        return html.Div([
            ...
            dcc.Graph(id='temperature-line', 
                      figure=px.line(weather_data, x="city", 
                                     y=["temperature_2m", "apparent_temperature"],
                                     title="Current Temperature vs. Feels-Like Temperature",
                                     labels={"value": "Temperature (°C)", "variable": "Temperature Type", "city": "City"},
                                     markers=True)),

            # Temperature Scatter Plot
            dcc.Graph(id='temperature-scatter',
                      figure=px.scatter(weather_data, x="city", y="temperature_2m",
            ...
            dcc.Graph(id='humidity-bar',
                      figure=px.bar(weather_data, x="city", y="relative_humidity_2m",
                                    color="relative_humidity_2m",
                                    title="Humidity Levels",
                                    labels={"relative_humidity_2m": "Humidity (%)", "city": "City"},
                                    color_continuous_scale="Blues")),

            # Dropdown + Wind Rose Graph
            html.Div([
                html.Label("Select City:", style={'display': 'flex', 'justify-self': 'self-end'}),
                dcc.Dropdown(
            ...
            dcc.Graph(id='cloud-cover-bar',
                      figure=px.bar(weather_data, x="city", y="cloud_cover",
                                    color="cloud_cover",
                                    title="Cloud Cover Percentage",
                                    labels={"cloud_cover": "Cloud Cover (%)", "city": "City"},
                                    color_continuous_scale="gray")),

            # Pressure Bar Chart
            dcc.Graph(id='pressure-bar',
                      figure=px.bar(weather_data, x="city", y="pressure_msl",
                                    color="pressure_msl",
                                    title="Pressure Levels",
                                    labels={"pressure_msl": "Pressure (hPa)", "city": "City"},
...
def update_wind_rose(selected_city):
    filtered_data = weather_data[weather_data["city"] == selected_city]

    fig = px.bar_polar(filtered_data, 
                       r="wind_speed_10m",  
                       theta="wind_direction_10m",  
                       color="wind_speed_10m",  
                       title=f"Wind Speed & Direction for {selected_city}",
